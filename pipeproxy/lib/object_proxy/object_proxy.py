...
class ObjectProxy:
    ...
    def __init__(self, proxy_message_transmitter: ProxyMessageTransmitter):
        """
        ...
        """
        self.proxy_message_transmitter = proxy_message_transmitter

    def send_message(self, function_name: str, args: tuple):
        """Creates a Request and sends it. Always expects a reply"""
        request = RequestMessage(function_name, args)
        reply = self.proxy_message_transmitter.send_message(request)
        assert isinstance(reply, ReplyMessage)
        return reply.get_content()

    @classmethod
    def add_method(cls, method: callable, name: str):
